...
// Total revenue
$stmt = $pdo->query("SELECT SUM(total_amount) FROM transactions WHERE status != 'cancelled'");
...
// Today's revenue
$stmt = $pdo->query("SELECT SUM(total_amount) FROM transactions WHERE DATE(created_at) = CURDATE() AND status != 'cancelled'");
